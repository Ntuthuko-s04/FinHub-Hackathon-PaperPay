class JSON_Editor extends HTMLElement {
  ...
  escape_html(input) {
    ...
    return replace.reduce(
      (escaped, replacement) => escaped.replaceAll(...replacement),
      input
    );
  }
  ...
  format_object(input, offset = 0) {
    ...
    output += Object.keys(input)
      .map((key, index, list) => {
        return `${"&nbsp;".repeat(
          offset + this.indent
        )}<span part="key" part="key"><span part="key_quotes">\"</span>${this.escape_html(
          key
        )}<span part="key_quotes">\"</span></span><span part="colon">:</span><span part="value">${this.format_input(
          input[key],
          offset + this.indent
        )}</span>${
  ...
  format_string(input) {
    return `<span part="string"><span part="string_quotes">\"</span>${this.escape_html(
      input
